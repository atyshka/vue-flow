...
export type FlowElement<NodeData = ElementData, EdgeData = ElementData, NodeEvents extends Record<string, CustomEvent> = any, EdgeEvents extends Record<string, CustomEvent> = any> = GraphNode<NodeData, NodeEvents> | GraphEdge<EdgeData, EdgeEvents>;
export type FlowElements<NodeData = ElementData, EdgeData = ElementData, NodeEvents extends Record<string, CustomEvent> = any, EdgeEvents extends Record<string, CustomEvent> = any> = FlowElement<NodeData, EdgeData, NodeEvents, EdgeEvents>[];
/** Initial elements (before parsing into state) */
export type Element<NodeData = ElementData, EdgeData = ElementData, NodeEvents extends Record<string, CustomEvent> = any, EdgeEvents extends Record<string, CustomEvent> = any> = Node<NodeData, NodeEvents> | Edge<EdgeData, EdgeEvents>;
export type Elements<NodeData = ElementData, EdgeData = ElementData, NodeEvents extends Record<string, CustomEvent> = any, EdgeEvents extends Record<string, CustomEvent> = any> = Element<NodeData, EdgeData, NodeEvents, EdgeEvents>[];
export type MaybeElement = Node | Edge | Connection | FlowElement | Element;
export interface CustomThemeVars {
    [key: string]: string | number | undefined;
}
export type CSSVars = '--vf-node-color' | '--vf-box-shadow' | '--vf-node-bg' | '--vf-node-text' | '--vf-connection-path' | '--vf-handle';
