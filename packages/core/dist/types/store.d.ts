...
import type { Dimensions, ElementData, Elements, FlowElements, FlowExportObject, FlowOptions, Rect, SelectionMode, SelectionRect, SnapGrid, XYPosition } from './flow';
import type { DefaultEdgeTypes, DefaultNodeTypes, EdgeComponent, NodeComponent } from './components';
import type { Connection, ConnectionLineOptions, ConnectionLineType, ConnectionMode, ConnectionStatus, Connector } from './connection';
...
export type RemoveNodes = (nodes: (string | Node) | (Node | string)[] | ((nodes: GraphNode[]) => (string | Node) | (Node | string)[]), removeConnectedEdges?: boolean) => void;
export type RemoveEdges = (edges: (string | Edge) | (Edge | string)[] | ((edges: GraphEdge[]) => (string | Edge) | (Edge | string)[])) => void;
export type AddEdges = (edgesOrConnections: (Edge | Connection) | (Edge | Connection)[] | ((edges: GraphEdge[]) => (Edge | Connection) | (Edge | Connection)[])) => void;
export type UpdateEdge = (oldEdge: GraphEdge, newConnection: Connection, shouldReplaceId?: boolean) => GraphEdge | false;
export type SetState = (state: Partial<FlowOptions & Omit<State, 'nodes' | 'edges' | 'modelValue'>> | ((state: State) => Partial<FlowOptions & Omit<State, 'nodes' | 'edges' | 'modelValue'>>)) => void;
...
export type FindNode = <Data = ElementData, CustomEvents extends Record<string, CustomEvent> = any>(id: string | undefined | null) => GraphNode<Data, CustomEvents> | undefined;
export type FindEdge = <Data = ElementData, CustomEvents extends Record<string, CustomEvent> = any>(id: string | undefined | null) => GraphEdge<Data, CustomEvents> | undefined;
export type GetIntersectingNodes = (node: (Partial<Node> & {
    id: Node['id'];
}) | Rect, partially?: boolean, nodes?: GraphNode[]) => GraphNode[];
export type IsNodeIntersecting = (node: (Partial<Node> & {
    id: Node['id'];
...
} & FlowHooksOn & ToRefs<State> & Readonly<ComputedGetters> & Readonly<Actions>;
