import type { Actions, Box, Connection, CoordinateExtent, DefaultEdgeOptions, Dimensions, Edge, EdgeMarkerType, ElementData, Elements, FlowElements, GraphEdge, GraphNode, MaybeElement, Node, Rect, ViewportTransform, XYPosition, XYZPosition } from "../types";
...
export declare function getOutgoers<T extends Elements = FlowElements>(node: Node, elements: T): T extends FlowElements<any, any, any, any> ? GraphNode<any, any, string>[] : Node<any, any, string>[];
export declare function getIncomers<T extends Elements = FlowElements>(node: Node, elements: T): T extends FlowElements<any, any, any, any> ? GraphNode<any, any, string>[] : Node<any, any, string>[];
 ...
 */
export declare function addEdge(edgeParams: Edge | Connection, elements: Elements, defaults?: DefaultEdgeOptions): Elements<any, any, any, any>;
/**
...
export declare function pointToRendererPoint({ x, y }: XYPosition, { x: tx, y: ty, zoom: tScale }: ViewportTransform, snapToGrid: boolean, [snapX, snapY]: [snapX: number, snapY: number]): XYPosition;
...
export declare function getNodesInside(nodes: GraphNode[], rect: Rect, { x: tx, y: ty, zoom: tScale }?: ViewportTransform, partially?: boolean, excludeNonSelectableNodes?: boolean): GraphNode<any, any, string>[];
export declare function getConnectedEdges<N extends Node | {
    id: string;
} | string, E extends Edge>(nodes: N[], edges: E[]): E[];
export declare function getConnectedNodes<N extends Node | {
    id: string;
} | string, E extends Edge>(nodes: N[], edges: E[]): N[];
export declare function getTransformForBounds(bounds: Rect, width: number, height: number, minZoom: number, maxZoom: number, padding?: number, offset?: {
    x?: number;
    y?: number;
}): ViewportTransform;
